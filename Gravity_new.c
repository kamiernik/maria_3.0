...
int areFieldGData     = 0; // Flag: are Gravi field data read?
...
void calc_GV_shift(int p_G_sites, double **p_G_data, double **p_G_results);
void calculate_gravi(block *p_net, model *calc_model, double **G_results_temp, int p_G_sites, double **p_G_data);
int count_sites(char *p_file);
void get_input_files(char *model_file, char *g_file, char *mg_file);
double Gravity(model *calc_model, block *p_net, int p_G_sites, double **p_G_data, double **p_G_results);
void open_G_data_auto(char *g_file, double **p_G_data);
...
void run_fwd_solvers(model *p_model1, block *p_net, int p_G_sites, double *p_GVmisfit, double **p_G_data, double **p_G_results);
...
{	
	int i = 0;
	double GV_field_avg = 0.0;
	double GV_model_avg = 0.0;
	double shift = 0.0;
	
	for(i=0; i<p_G_sites; i++) 
	{                  
		GV_field_avg += p_G_data[i][1]; 
		GV_model_avg += p_G_results[i][1];
	} 
	 
	GV_field_avg = GV_field_avg / p_G_sites;
	GV_model_avg = GV_model_avg / p_G_sites;
	 
	isShiftCalculated = 1;
	 
	if(GV_field_avg > GV_model_avg)
	{
		shift = GV_field_avg - GV_model_avg;
	...
	{		
		shift = GV_model_avg - GV_field_avg;
...
{	
	...
	double x0, x1, x2, z1, z2, r1sq, r2sq, alpha, beta, factor, term1, term2, sum, denom, g;
	
	if(G_results_temp == NULL) 
	{ 
		g_print("\n Gravi: Przydzielenie pamieci dla wynikow nie bylo mozliwe"); 
		getchar(); 
	} 
	
	for(j=0; j<p_G_sites; j++) // obliczanie efektu dla pojedynczego punktu pomiarowego
	{
		x0 = p_G_data[j][0];
		...
		{ 			
			...
			{	  
				...
				// wyliczenie efektu grawitacyjnego 
				if(x1 == 0)
		    		x1 = 1e-12;
				if(x2 == 0)
					x2 = 1e-12;
			...
			g = g + 2*sum*p_net[i].rhob*si2mg*gamma;	// efekt koncowy z oczka
		}
				
		G_results_temp[j][0] = x0;
		G_results_temp[j][1] = g; // wynik					
	}	
...
{	
	...
	double *dwk4G; 	 	     // tablica przechowujaca roznice miedzy spreparowanymi danymi polowymi a rzeczywistymi (dla pozniejszego RMSE) // KM
	double **G_results_temp;
	struct timeval tic, toc;

	gettimeofday(&tic, NULL);
	 	
	dwk4G = (double*)malloc(p_G_sites * sizeof(double));	
	
	G_results_temp = (double**)malloc((p_G_sites)*sizeof(double*)); // alokacja pamieci na wyniki do MainForma
	for(i=0; i<p_G_sites; i++) 
		G_results_temp[i] = (double*)malloc(2*sizeof(double));
	
	calculate_gravi(p_net, calc_model, G_results_temp, p_G_sites, p_G_data); // fun. liczaca ef. grawitacyjny

	if(areFieldGData) // misfit ma sens tylko przy wczytaniu danych polowych
	{
		for(i=0; i<p_G_sites; i++) 
		{                    
			dwk4G[i] = p_G_data[i][1] - G_results_temp[i][1];         
		} 
		
		misfit = anorm(p_G_sites, dwk4G);
	}
	else 
		misfit = DBL_MAX; //1.0;	
	...
	for(i=0; i<p_G_sites; i++) 
	{
		p_G_results[i][0] = G_results_temp[i][0];
		p_G_results[i][1] = G_results_temp[i][1]; 
	}
	...
	for(i=0; i<p_G_sites; i++)
		g_free(G_results_temp[i]);
	g_free(G_results_temp);
...
void open_G_data_auto(char *g_file, double **p_G_data)
{
	...
	FILE *data_GV;
	data_GV = fopen(g_file, "r");

	int i = 0;  	
	int j = 0;	
	int res = 0;									
		
	res = fscanf(data_GV, "%d", &p_G_sites); 

	if(res < 1) // nie wczytal ilosci stacji
		return;
								
	G_data_temp = (double**)malloc(p_G_sites*sizeof(double*)); // alokacja pamieci na dane polowe
	for(j=0; j<p_G_sites; j++)
		G_data_temp[j]=(double*)malloc(2*sizeof(double));
	...
	for(i=0; i<p_G_sites; i++) // wczytanie danych polowych
	{
		res = fscanf(data_GV, "%lf %lf", &G_data_temp[i][0], &G_data_temp[i][1]); 
	...
	for(i=0; i<p_G_sites; i++)
	{
		p_G_data[i][0] = G_data_temp[i][0];
		p_G_data[i][1] = G_data_temp[i][1];
	}		
		
	for(i=0; i<p_G_sites; i++)
        g_free(G_data_temp[i]);
    g_free(G_data_temp);
    G_data_temp = NULL; 
...
{		
	...
	{			
		if(GVIsIn) *p_GVmisfit = Gravity(p_model1, p_net, p_G_sites, p_G_data, p_G_results);	
		
		if(isShiftCalculated == 0)
        {		
			calc_GV_shift(p_G_sites, p_G_data, p_G_results); 
		    *p_GVmisfit = Gravity(p_model1, p_net, p_G_sites, p_G_data, p_G_results);
        }	
        areFwdSolvers = 1;				
	}
	else
	{
		if(GVIsIn) *p_GVmisfit = DBL_MAX; // 1.0;	
        areFwdSolvers = 0; //?
	}
    if(*p_GVmisfit < DBL_MAX)
	{
		g_print("G Misfit = %lf ", *p_GVmisfit);
...
{   
	int i = 0;
	int G_sites = 0;  				// il. pkt. pomiarowych gravi
	char model_file[120] = "";
	char g_file[120] = "";
	char mg_file[120] = "";	

	double GVmisfit	= DBL_MAX; 
	double **G_data; 	  // site localization [i][0] and field data [i][1] for Gravi - field data ok
	double **G_results;   // site localization [i][0] and model response [i][1] for Gravi
	...
	isShiftCalculated = 0;
	areFieldGData     = 0; // Flag: are Gravi field data read?
	...
	G_sites = count_sites(g_file);
	alloc_2D_array(&G_data, G_sites, 2);
	alloc_2D_array(&G_results, G_sites, 2);

	open_G_data_auto(g_file, G_data);
	...
	run_fwd_solvers(model1, net1, G_sites, &GVmisfit, G_data, G_results);
